...
void BootLoaderScreen::render()
{
    ...
    {
        if (i == entry_curr)
            m_framebuffer.draw_filled_rectangle(0, 10 * i, 320, 10,
                44);
        // file.render(m_framebuffer);
        sprintf(file_string_temp, "%c | %dKB | %s", entries[i].is_dir ? 'D' : 'F', entries[i].file_size / 1000, entries[i].name);
        m_framebuffer.draw_string(file_string_temp, 0, 10 * i, 15);
    }

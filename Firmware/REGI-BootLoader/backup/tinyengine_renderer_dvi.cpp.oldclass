...
static TinyEngineFrameBuffer* s_frame_buf;
...
tinyengine_status_t __not_in_flash_func(TinyEngineRendererDVI::wait_for_vsync()) {
    // while (v_scanline >= MODE_V_FRONT_PORCH) __wfe();
    while (v_scanline >= MODE_V_FRONT_PORCH) __wfe();
}


tinyengine_status_t __not_in_flash_func(te_renderer_dvi_wait_vsync_blocking()) {
...
tinyengine_status_t __not_in_flash_func(TinyEngineRendererDVI::swap_blocking()) {
    wait_for_vsync();
